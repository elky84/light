...
void relay_server::pve_subscribe_handler(const std::vector<char> &buf)
{
...
void relay_server::pve_list_subscribe_handler(const std::vector<char> &buf)
{
    std::string msg(buf.begin(), buf.end());
    LOG_DEBUG(L"%S", msg.c_str());
...
bool relay_server::initialize()
{
    ...
    m_pve_subscriber->asyncConnect(address, m_port, [&](bool status, const std::string &err)
    {
        ...
        else
        {
            ...
            m_pve_subscriber->subscribe(pve_channelName, boost::bind(&relay_server::pve_subscribe_handler, this, _1));
        }
    });
    ...
    m_pve_list_subscriber->asyncConnect(address, m_port, [&](bool status, const std::string &err)
    {
        ...
        else
        {
            ...
            m_pve_list_subscriber->subscribe(req_pve_list_channelName, boost::bind(&relay_server::pve_list_subscribe_handler, this, _1));
        }
    });
...
light::net::PeerEx* relay_server::peer_allocator(int idx)
{
