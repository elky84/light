...
// read_file fseek, ftell and offset type
#if defined(__GNUC__) && ! defined(MARKUP_WINDOWS) // non-Windows GNUC
...
#define MCD_EOL MCD_T("\r\n")
#else // not Windows and not otherlight specified
#define MCD_EOL MCD_T("\n")
#endif // not Windows and not otherlight specified
...
public:
	...
	MCD_STR get() { return x_get(m_iPos); };
	MCD_STR GetChildData() { return x_get(m_iPosChild); };
	...
	const MCD_STR& GetResult() const { return m_Result; };
	...
	const MCD_STR& GetDoc() const { return m_Doc; };
	...
	bool Set( MCD_CSTR szData, int nFlags=0 ) { return x_Set(m_iPos,szData,nFlags); };
	bool SetChildData( MCD_CSTR szData, int nFlags=0 ) { return x_Set(m_iPosChild,szData,nFlags); };
	bool Set( int nValue ) { return x_Set(m_iPos,nValue); };
	bool SetChildData( int nValue ) { return x_Set(m_iPosChild,nValue); };
	...
	static int UTF16To8( char *UTF8, const unsigned short* pwszUTF16, int nUTF8Count );
	static int UTF8To16( unsigned short* pwszUTF16, const char* UTF8, int nUTF8Count );
	static MCD_STR UTF8ToA( MCD_CSTR UTF8, int* pnFailed = NULL );
	static MCD_STR AToUTF8( MCD_CSTR ANSI );
	static void EncodeCharUTF8( int nUChar, char* UTF8, int& nUTF8Len );
	static int DecodeCharUTF8( const char*& UTF8, const char* UTF8End = NULL );
	static void EncodeCharUTF16( int nUChar, unsigned short* pwszUTF16, int& nUTF16Len );
	static int DecodeCharUTF16( const unsigned short*& pwszUTF16, const unsigned short* UTF16End = NULL );
	...
	static int GetEncodingCodePage( MCD_CSTR Encoding );
...
protected:
	...
	FilePos* m_FilePos;
	SavedPosMapArray* m_SavedPosMaps;
	ElemPosTree* m_ElemPosTree;
...
#define MARKUP_SETDEBUGSTATE
	...
	bool x_Set( int iPos, MCD_PCSZ szData, int nFlags );
	bool x_Set( int iPos, int nValue );
